...
class Get_Gateway_Cellular(unittest.TestCase):
    def setup(self):
    ...
    def test_get_gateway_cellular(self):
        print("\nStarting Get LTE Gateway Cellular test...\n\n")
        log_file.write("\nStarting Get LTE Gateway Cellular test...\n\n")
        ...
        print(f"LTE gateway id to get Cellular data: {gateway_id}\n")
        log_file.write(f"LTE gateway id to get Cellular data: {gateway_id}\n")
        # Send get request
        ...
        if rGet.status_code == 200:
            print("\nGet LTE Gateway Cellular Test passed. The Response code is: ")
            print(str(rGet.status_code))
            log_file.write("\nGet LTE Gateway Cellular Test passed. The Response code is: ")
            log_file.write(str(rGet.status_code))
            self.assertEqual(rGet.status_code, 200)
        else:
            print("Get LTE Gateway Cellular Test failed. The Response code is not 200 (OK) and it is: ")
            log_file.write("Get LTE Gateway Cellular Test failed. The Response code is not 200 (OK) and it is: ")
            print(str(rGet.status_code))
            log_file.write(str(rGet.status_code))
            self.assertEqual(rGet.status_code, 200)
    ...
    def tearDown(self):
        time.sleep(5)
        print("\n\nGet LTE Gateway Cellular Test ended\n")
        log_file.write("\n\nGet LTE Gateway Cellular Test ended\n")
        print(f"\nReport stored at:\n{log_file_path}\n")
        log_file.close()
