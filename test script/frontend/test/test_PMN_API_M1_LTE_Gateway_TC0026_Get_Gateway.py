...
class Get_Gateway(unittest.TestCase):
    def setup(self):
    ...
    def test_get_gateway(self):
        print("\nStarting Get LTE Gateway test...\n\n")
        log_file.write("\nStarting LTE Get Gateway test...\n\n")
        cert_path = root_dir + values['cert']
        key_path = root_dir + values['key']
        network_gateway = user_values['network_gateway']
        # Send get request
        rGet = requests.get(url=(values['url'] + f"lte/{network_gateway}/gateways"), cert=(cert_path, key_path), verify=False)
        print("\nResponse body:\n" + str(rGet.text))
        log_file.write("\nResponse body:\n" + str(rGet.text))

        if rGet.status_code == 200:
            print("\nGet LTE Gateway Test passed. The Response code is: ")
            print(str(rGet.status_code))
            log_file.write("\nGet LTE Gateway Test passed. The Response code is: ")
            log_file.write(str(rGet.status_code))
            self.assertEqual(rGet.status_code, 200)
        else:
            print("Get LTE Gateway Test failed. The Response code is not 200 (OK) and it is: ")
            log_file.write("Get LTE Gateway Test failed. The Response code is not 200 (OK) and it is: ")
            print(str(rGet.status_code))
            log_file.write(str(rGet.status_code))
            self.assertEqual(rGet.status_code, 200)
