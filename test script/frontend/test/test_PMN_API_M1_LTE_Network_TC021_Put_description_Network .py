...
class Put_description_network(unittest.TestCase):
    ...
    def test_put_apn(self):
        print("\nStarting Put description test:\n")
        log_file.write("\nStarting Put description test\n")
        cert_path = root_dir + values['cert']
        key_path = root_dir + values['key']
        json_path = root_dir + values['json_put_description']
        network = 'test1'

        rPut = requests.put(url=(values['url'] + f"lte/{network}/description"), cert=(cert_path, key_path), verify=False)
        print(str(rPut.text))
        log_file.write(str(rPut.text))
        if rPut.status_code == 204:
            print("\nPut description Network Test passed. The response code is:\n")
            print(str(rPut.status_code))
            log_file.write("\nPut description Network Test passed. The response code is:\n")
            log_file.write(str(rPut.status_code))
            self.assertEqual(rPut.status_code, 204)
        else:
            print("Put description Network Test failed. The response code  is:\n")
            log_file.write("Put description Network Test failed. The response code is:\n")
            print(str(rPut.status_code))
            self.assertNotEqual(rPut.status_code, 204)
            log_file.write(str(rPut.status_code))
    ...
    def tearDown(self):
        ...
        time.sleep(5)
        print("\nPut description Network Test ended\n")
        log_file.write("\nPut description Network Test ended\n")
        print(f"\nReport stored at:\n{log_file_path}\n")
        log_file.close()
