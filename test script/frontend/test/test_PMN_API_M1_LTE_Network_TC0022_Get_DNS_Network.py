...
class Get_DNS_Network (unittest.TestCase):
    def setup(self):
        self.url = values['url']
    ...
    def test_get_dns_network(self):
        print("Starting  get network dns test:\n")
        log_file.write("Starting  get network dns test\n")
        cert_path = root_dir + values['cert']
        key_path = root_dir + values['key']
        print("Starting get test:")
        rGet = requests.get(url=(values['url']+"lte/test1/dns"),cert=(cert_path, key_path), verify=False)
        print(rGet.text)
