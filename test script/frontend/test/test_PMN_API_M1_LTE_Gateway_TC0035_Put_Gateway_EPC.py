...
class Put_Gateway_EPC(unittest.TestCase):
    ...
    def test_put_gateway_epc(self):
        print("\nStarting Put LTE Gateway EPC test:\n")
        log_file.write("\nStarting Put LTE Gateway EPC test\n")
        cert_path = root_dir + values['cert']
        key_path = root_dir + values['key']
        json_path = root_dir + values['json_put_gateway_epc']
        ...
        print (f"Editing LTE Gateway EPC data: {gateway_id}")
        rPut = requests.put(url=(values['url'] + f"lte/{network_gateway}/gateways/{gateway_id}/cellular/epc"), cert=(cert_path, key_path), json=json_data_obj, verify=False)
        print(rPut.text)
        log_file.write(rPut.text)
        if rPut.status_code == 204:
            print("\nPut LTE Gateway EPC Test passed. The response code is:\n")
            print(str(rPut.status_code))
            log_file.write("\nPut LTE Gateway EPC Test passed. The response code is:\n")
            log_file.write(str(rPut.status_code))
            self.assertEqual(rPut.status_code, 204)
        else:
            print("Put LTE Gateway EPC Test failed. The response code  is:\n")
            log_file.write("Put LTE Gateway EPC Test failed. The response code is:\n")
            print(str(rPut.status_code))
            log_file.write(str(rPut.status_code))
            self.assertEqual(rPut.status_code, 204)

        # verify the edited LTE Gateway EPC data using get command
        print("\n\nVerifying edited LTE Gateway EPC using GET command:\n")
        log_file.write("\n\nVerifying edited LTE Gateway EPC using GET command:\n")
        time.sleep(5)
        rGet = requests.get(url=(values['url'] + f"lte/{network_gateway}/gateways/{gateway_id}/cellular/epc"), cert=(cert_path, key_path), verify=False)
        if rGet.status_code == 200:

            print(f"\nLTE Gateway EPC {gateway_id} is edited as expected. \n")
            log_file.write(f"\nLTE Gateway EPC {gateway_id} is edited as expected. \n")
            print(rGet.text)
            log_file.write(rGet.text)
            self.assertEqual(rGet.status_code, 200)
        else:
            print(f"\nLTE Gateway EPC {gateway_id} is not edited. \n ")
            log_file.write(f"\nLTE Gateway EPC {gateway_id}is not edited. \n ")
            self.assertEqual(rGet.status_code, 200)
    ...
    def tearDown(self):
        ...
        time.sleep(5)
        print("\nPut LTE Gateway EPC Test ended\n")
        log_file.write("\nPut LTE Gateway EPC Test ended\n")
        print(f"\nReport stored at:\n{log_file_path}\n")
        log_file.close()
