...
class network_Get_RAN_configuration(unittest.TestCase):
    def setup(self):
        self.url = values['url']
        self.cert = values['cert']
        self.key = values['key']

        print("Start Get Network RAN configuration setup")
        print("End Get Network RAN configuration setup")

    def test_get_RAN_configuration_network(self):
        print("Starting  get Network RAN configuration test:\n")
        log_file.write("Starting  get Network RAN configuration test\n")
        cert_path = root_dir + values['cert']
        key_path = root_dir + values['key']

        rGet = requests.get(url=(values['url'] + "lte/test1/cellular/ran"), cert=(cert_path, key_path), verify=False)
        print(str(rGet.text))
        log_file.write(str(rGet.text))
        if rGet.status_code == 200:
            self.assertEqual(rGet.status_code, 200)
            print("Get network RAN configuration passed. The current status:\n")
            print(str(rGet.status_code))
            log_file.write("Get network RAN configuration passed. The current status:\n")
            log_file.write(str(rGet.status_code))
        else:
            self.assertEqual(rGet.status_code, 200)
            print("Get Network RAN configuration  failed. The current status is not 200 (OK) and it is:\n ")
            log_file.write("Get Network RAN configuration  failed. The current status is not 200 (OK) and it is:\n ")
            print(str(rGet))
    ...
    def tearDown(self):
        ...
        time.sleep(5)
        print("\nGet Network RAN configuration  ended\n")
        log_file.write("\nGet Network RAN configuration  ended\n")
        log_file.close()
