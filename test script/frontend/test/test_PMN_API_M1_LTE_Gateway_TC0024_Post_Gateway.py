...
class Post_Gateway(unittest.TestCase):
    ...
    def test_post_gateway(self):
        print("\nStarting Post LTE Gateway test:\n")
        log_file.write("\nStarting Post LTE Gateway test\n")
        cert_path = root_dir + values['cert']
        key_path = root_dir + values['key']
        json_path = root_dir + values['json_add_gateway']
        ...
        rPost = requests.post(url=(values['url'] + f"lte/{network_gateway}/gateways"), cert=(cert_path, key_path), json=json_data_obj, verify=False)
        print(rPost.text)
        log_file.write(rPost.text)
        if rPost.status_code == 201:
            print("\nPost LTE Gateway Test passed. The response code is:\n")
            print(str(rPost.status_code))
            log_file.write("\nPost LTE Gateway Test passed. The response code is:\n")
            log_file.write(str(rPost.status_code))
            self.assertEqual(rPost.status_code, 201)
        else:
            print("Post LTE Gateway Test failed. The response code  is:\n")
            log_file.write("Post LTE Gateway Test failed. The response code is:\n")
            print(str(rPost.status_code))
            log_file.write(str(rPost.status_code))
            self.assertEqual(rPost.status_code, 201)

        # verify the created Gateway is present using get command
        print("\n\nVerifying added LTE Gateway is present:\n")
        log_file.write("\n\nVerifying added LTE Gateway is present:\n")
        time.sleep(5)
        rGet = requests.get(url=(values['url'] + f"lte/{network_gateway}/gateways/{gateway_id}"), cert=(cert_path, key_path), verify=False)
        if rGet.status_code == 200:

            print(f"\nLTE Gateway {gateway_id} is added as expected. \n")
            log_file.write(f"\nLTE Gateway {gateway_id} is added as expected. \n")
            print(rGet.text)
            log_file.write(rGet.text)
            self.assertEqual(rGet.status_code, 200)
        else:
            print(f"\nLTE Gateway {gateway_id} is not added. \n ")
            log_file.write(f"\nLTE Gateway {gateway_id}is not added. \n ")
            self.assertEqual(rGet.status_code, 200)
