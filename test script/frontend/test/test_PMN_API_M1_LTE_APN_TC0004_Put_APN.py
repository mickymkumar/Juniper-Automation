...
class Put_APN(unittest.TestCase):
    ...
    def test_put_apn(self):
        ...
        # verify the edited APN using GET command
        print("\n\nVerifying edited APN using GET command:\n")
        log_file.write("\n\nVerifying edited APN using GET command:\n")
        time.sleep(5)
        rGet = requests.get(url=(values['url'] + f"lte/{network_apn}/apns/{apn_name}"), cert=(cert_path, key_path),
                                verify=False)
